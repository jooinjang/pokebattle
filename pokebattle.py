#!/usr/bin/env python3
"""1~2세대 포켓몬 251마리 중 타입 상성만으로 5마리 엔트리를 고른다.
...
사용법:
    ...
    pokebattle.py --rank                       # 251마리를 타입 상성 강도순으로 나열
    pokebattle.py --maxev                      # 상대 251마리 균등 가정의 최대기대값 엔트리
...
def nash_multiset_dist(k, caps):
    ...
    # 정원에 딱 붙는 조합은 확률적으로 뽑을 여지가 없다. 균형이 요구하는 기대
    # 개수가 정원과 같으면(예: 드래곤/비행 20% x 5슬롯 = 1, 망나뇽 1마리뿐)
    # 그 조합은 항상 정원만큼 나와야 하고, IPF 가중치는 발산한다.
    # 그런 조합을 먼저 고정하고 남은 슬롯만 IPF로 맞춘다.
    forced = [0] * len(p)
    left, kk = list(p), k
    while True:
        hit = [i for i in range(len(p))
               if left[i] > 0 and kk * left[i] >= caps[i] - forced[i] - 1e-12]
        if not hit or kk == 0:
            break
        for i in hit:
            forced[i] = caps[i]
        need = {i: k * p[i] for i in range(len(p)) if left[i] > 0 and i not in hit}
        kk = k - sum(forced)
        left = [0.0] * len(p)
        if kk > 0:
            s = sum(need.values())
            for i, v in need.items():
                left[i] = v / s
    if sum(forced) == k:
        return [(1.0, tuple(forced))]
    p, base = left, forced
    k = k - sum(forced)
    caps = [caps[i] - forced[i] for i in range(len(caps))]
    ...
    for _ in range(200):
        ...
        s = sum(w)
        w = [x / s for x in w]
    return [(pr, tuple(base[i] + v[i] for i in range(len(base)))) for pr, v in zip(probs, vecs)]
...
def resolve(token):
    ...
    raise SystemExit(f"모르는 포켓몬: {token!r} (1~2세대 한국어 이름 또는 1~251 도감번호)")
...
def print_ranking():
    rows = ranking()
    print(f"1~2세대 {len(MONS)}마리 타입 상성 강도순 — 전체와 1:1로 붙였을 때의 성적")
...
def print_maxev(dup=False):
    ...
    print(f"타입 조합별 개별 기대점수 (상대 = {len(MONS)}마리 균등, 승1 무0.5 패0)")
    print()
    team = []
...
def selftest():
    ...
    assert len(MONS) == 251 and len(CHART) == 18 and len(COMBOS) == 60
    print("selftest ok")
    return 0
